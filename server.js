...
const authLimiter = rateLimit({ windowMs: 30 * 60 * 1000, max: 5, message: { error: 'Too many login attempts' } });

app.use(apiLimiter);
app.use(express.json({ limit: '10mb' }));
app.use(express.static('public', { maxAge: NODE_ENV === 'production' ? '1d' : '0' }));
app.use('/uploads', express.static(ASSETS_PATH, { maxAge: NODE_ENV === 'production' ? '7d' : '0' }));

app.use(session({
    store: new FileStore({ path: SESSIONS_PATH, ttl: 24 * 60 * 60, retries: 2, logFn: () => {} }),
    secret: process.env.SESSION_SECRET || 'zgallery_secret_' + Math.random().toString(36).slice(2),
    resave: false,
    saveUninitialized: false,
    cookie: { secure: NODE_ENV === 'production', httpOnly: true, maxAge: 24 * 60 * 60 * 1000, sameSite: 'lax' }
}));
...
app.post('/api/logout', (req, res) => {
    req.session.destroy((err) => {
        ...
        res.clearCookie('connect.sid', { path: '/', httpOnly: true, secure: NODE_ENV === 'production', sameSite: 'lax' });
        res.json({ success: true });
    });
});
...
app.get('/health', (req, res) => {
    res.json({ status: 'ok', uptime: process.uptime(), memory: process.memoryUsage().heapUsed / 1024 / 1024 + ' MB', env: NODE_ENV });
});
